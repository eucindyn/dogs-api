...
function api_photo_delete($request) {
  ...
  $user_id = (int) $user->ID;
  // colocando o (int) ele sempre retorna um valor integral - número

  if($user_id !== $author_id || !isset($post)) {
    $response = new WP_Error("error", "no permission", ["status" => 401]);
  ...
  return rest_ensure_response('deleted post');
}
